...
const DebugOverlay: React.FC = () => {
  ...
  const fetch = useCallback(async () => {
    const sent = Date.now();
    const { resolvedAt } = await updateList(
      async () => {
  ...
  return (
    <div>
      ...
      <a
        href={
          "https://github.com/Censkh/react-async-stateful/blob/master/examples/views/DebugOverlay.tsx"
        }
      >
        Source Code
      </a>
      ...
      <AsyncStateOverlay
        debug={debug}
        state={list}
        style={{ border: "1px solid #ddd", padding: "5px", width: "500px" }}
      >
        {list => (
          <div>
            <label>Debug:</label>
            ...
            <input
              ...
              max={12}
            />
            <button disabled={list.pending} onClick={fetch}>
              Fetch
            </button>
            <br />
            <b>List of {list.value?.length} value(s):</b>{" "}
            {list.pending && <i>Loading...</i>}
            <ul>
